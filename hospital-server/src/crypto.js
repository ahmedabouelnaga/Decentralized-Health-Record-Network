...
export function privateKeyToPublicKeyBytes(privateKeyHex) {
  const privBytes = fromHex(privateKeyHex);
  return secp256k1.getPublicKey(privBytes, false); // 65 bytes uncompressed
}

export async function eciesEncrypt(recipientPubKeyHex, plaintext) {
  const recipientPub = typeof recipientPubKeyHex === 'string'
    ? fromHex(recipientPubKeyHex)
    : recipientPubKeyHex;
